...
def get_user_data(uid):
    ...
    if not os.path.exists(file):
        data = {
            ...
            "ref_notified": False  # Уведомление о реферале отправлено
        }
...
async def ref_test(msg: types.Message, state):
    """Команда для тестирования реферальной системы (только для админа)"""
    uid = msg.from_user.id
    
    # Проверяем админа (можно добавить список админов)
    user_data = get_user_data(uid)
